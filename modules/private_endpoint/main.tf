...
// create private endpoint for event hub to datalake gen2
resource "azurerm_private_endpoint" "eventhub_datalake_gen2_private_endpoint" {
  name                = "eventhub-datalake-gen2-private-endpoint"
  location            = var.location
  resource_group_name = var.resource_group_name
  subnet_id           = var.subnet_ids[5]

    private_service_connection {
        name                           = "eventhub-datalake-gen2-psc"
        is_manual_connection            = false
        private_connection_resource_id   = var.datalake_storage_account_id
        subresource_names               = ["dfs"]
    }
...
resource "azurerm_private_dns_zone" "eventhub_datalake_gen2_private_dns_zone" {
  name                = "privatelink.servicebus.windows.net"
  resource_group_name = var.resource_group_name
}

resource "azurerm_private_dns_zone_virtual_network_link" "eventhub_datalake_gen2_dns_zone_link" {
  name                  = "eventhub-datalake-gen2-dns-zone-link"
  resource_group_name   = var.resource_group_name
  private_dns_zone_name = azurerm_private_dns_zone.eventhub_datalake_gen2_private_dns_zone.name
  virtual_network_id    = var.vnet_id
}
